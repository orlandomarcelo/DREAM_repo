...
ex = Experiment('autofocus_sharp', ingredients=[pulse_ingredient, start_and_stop])
...
def send_IBPC(_run, name, limit_blue, gain, exposure, limit_green, limit_purple, limit_red, trigger_color, sample_rate, acq_time, period_SP, length_SP, period_ML, actinic_filter, move_plate):
    ...
    def focus(min, max, step, link_arduino, exposure, gain):
        ...
        if y == 0:
            ...
            v_min = voltages[y-1]
        if y == step-1:
            v_max = voltages[step-1]
        else: 
            v_max = voltages[y + 1]
        return v_min,  v_max, voltages, blurs, images
    ...
    with open("G:/DREAM/from_github/PAMFluo/specs/focus.txt", 'w') as f:
