...
class Heterodyne(NIControl.AcquisitionClass.Acquisition):

    def __init__(self):
        super().__init__()
        
    
    def fixed_dw(self, frequencies, dw, offset, amplitude, ratio, sleep_time):
        N = len(frequencies)
        ...
        for i, freq in enumerate(frequencies):
            ...
            self.num_period = int(frequencies.max()/dw)*10
            self.points_per_period = 200
            print(self.num_period)
            self.update_rates() 
            self.offset =  0
            self.amplitude = 0
            signal_480 = self.sine_pattern()#self.sine_pattern_dw(dw, ratio, 1-ratio)
            #plt.figure()
            #plt.plot(signal_dw)
            #plt.show()
            ...
            signal_405 = self.sine_pattern_dw(dw, ratio, 1-ratio)

            signal_dw = np.stack((signal_480, signal_405), axis = 0)


            self.generator_analog_init(signal_dw)
            ...
            sin_lock, cos_lock, radius_lock, phase_lock  = self.lock_in(output, time_array, dw, phase_shift = phase_shift)
            ...
            self.end_tasks()
            self.set_level_LED(signal_dw.mean(axis = 1), None, sleep_time)
        ...
        self.set_level_LED(0*signal_dw.mean(axis = 1), None, sleep_time)       
        return radius, phase, all_outputs_2D, sin_lo, cos_lo, all_outputs_3D, all_times


 
    def LEDs_2_dw(self, frequencies, dw, offset, amplitude, ratio, sleep_time):
        N = len(frequencies)
        radius = np.zeros((N, self.num_channels_analog() + self.num_channels_digital()))
        phase = np.zeros((N, self.num_channels_analog() + self.num_channels_digital()))
        sin_lo = np.zeros((N, self.num_channels_analog() + self.num_channels_digital()))
        ...
        for i, freq in enumerate(frequencies):
            ...
            self.num_period = int(frequencies.max()/dw)*20
            self.points_per_period = 200
            print(self.num_period)
            self.update_rates() 
            print(offset, amplitude)
            ...
            self.offset = offset
            self.amplitude = amplitude
            signal_405 = self.sine_pattern_modified_dw(dw)
