...
class FW102C(object):
    ...
    def move_to_filter(self, pos):
        actual_pos = self.query("pos?")
        if actual_pos != str(pos):
            print('**pos=%d'%pos, self.command('pos=%d'%pos))
            new_pos = int(self.query("pos?"))

            print("Position of wheel:", new_pos)
            if new_pos != pos:
            ...
            print("Wheel already in position")
