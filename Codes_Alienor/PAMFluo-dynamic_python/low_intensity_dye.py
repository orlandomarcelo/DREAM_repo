...
@ex.config
def my_config():
    name = "low_intensity_DCM_calib_cam"
...
def activation_curve(_run, name, gen_ana, sleep_time, N, detect_input, limit_blue, limit_red, limit_green, 
    ...
    def fitness_sequence(ni, offset, start, stop):
    
        sample = np.linspace(0, ni.acq_time, num = ni.writing_samples , endpoint=False)
        signal = np.zeros(sample.shape)
        signal[start*ni.writing_rate:stop*ni.writing_rate] = offset
    ...
    for i in range(len(offsets)):
        ...
        sample, signal = fitness_sequence(ni, offsets[i], start, stop)
