...
def send_IBPC(_run, _log, name, limit_blue, limit_green, limit_red, limit_purple, trigger_color, sample_rate, acq_time, 
        length_SP, length_ML, period_ML, exposure, focus_pos ,gain):
    ...
    add_digital_pulse(link, pins[trigger_color], 1000, 4*minute//10, minute//10, 0) 

    add_digital_pulse(link, pins["blue"], 1000, 10*minute, 10*minute, 0)
    add_digital_pulse(link, pins["no_LED_trig"], 1000, 200, 20, 0)
    ...
    ni.p.xlabel = "time"
    ni.p.ylabel = 'fluo'
    fig = ni.p.plotting(timing, np.mean(video_downscaled, axis = 1))
    ni.p.saving(fig)

    trig = output[arduino_purple]
    points_per_period = 60//2
    tau_lists = Parallel(n_jobs = -1 )(get_tau(trig, video_downscaled[:,i], timing, 3.3, points_per_period, sample_rate, residuals, exp_decay, 0.5) for i in range(video_downscaled.shape[1]))
    #for i in range(L):        
    #    for j in range(H):
    ...
    tau_480_map = tau_lists[:,2]
    tau_480_std = tau_lists[:,3]
    tau_405_map = tau_lists[:,0]
    ...
    I_480_map[I_480_map >= 1.5*I_480_map.mean() ] = 1 


    I_405_map[I_405_map <=0 ] = 1
    I_405_map[I_405_map >= 1.5*I_405_map.mean() ] = 1 
