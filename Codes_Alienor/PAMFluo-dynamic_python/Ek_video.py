...
def my_config():
    ...
    acq_time = (n_samples * period_SP + 3) * (N//2)*2 + len(filter_list)*15
...
def SP_calibration(_run, name, limit_blue, limit_green, limit_purple, trigger_color, sample_rate, frame_rate, 
    ...
    add_master_digital_pulse(link, pins['no_LED_trig'], 0+delay_trig, period_SP*sec, 20, 0) #trigger
    ...
    #limits_blue_low = np.linspace(1, 45, N//(2*len(filter_list)))
    ...
    limits_blue = np.array([0,2,10,30, 60, 120, 240, 500])
    thread.start()
    ni.detector_start()
    ...
    for filter in filter_list:
        ...
        for limit_blue in limits_blue:
            ...
            time.sleep(n_samples*period_SP-0.1)
            stop_measurement(link)

            add_digital_pulse(link, pins['blue'], 500, period_SP*sec, length_SP*sec, 0)
            add_master_digital_pulse(link, pins['no_LED_trig'], 500+delay_trig, period_SP*sec, 20, 0) #trigger
