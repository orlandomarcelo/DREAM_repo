...
@ex.config
def my_config():
    name = "Activation_DCM_pad_cam_60s_dark_chauffe"
...
def activation_curve(_run, name, gen_ana, sleep_time, N, detect_input, limit_blue, limit_red, limit_green, 
    ...
    for i in range(len(offsets)):
        ...
        ######## Performs another acquisitionn in the dark for 60 s ########

        thread, cam = init_camera(exposure = exposure, num_frames = 15*60, gain =  gain, trigger=False, trigger_frequency=1000/exposure)
        thread.start()

        signal = np.zeros(np.linspace(0, 60, num = ni.writing_samples , endpoint=False).shape)
